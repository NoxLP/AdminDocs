...
export default function useGallery() {
  ...
  const [autocompleteItems, setAutocompleteItems] = useState<
    Array<TAutocompleteDropdownItem>
  >([]);
...
${doc.category}`;
  ...
  const documentIncludes = (document: IDocument, value: any): boolean => {
    ...
    return (
      document.date.toLocaleString().includes(value) ||
      Object.values(DocumentCategory).some((enumValue) =>
        enumValue.includes(value)
      ) ||
  ...
  const filterDocuments = (
    ...
    if (search.length == 0) setFilteredDocuments(documents);
    else {
      const docs = [...documents];
      if (!isId)
        setFilteredDocuments(
          docs.filter((doc) => documentIncludes(doc, search))
        );
      else
